...
validate n
    | luhn (toDigits (n `div` 10)) == ((fromIntegral n) `mod` 10) = True
    | otherwise = False
...
luhn :: [Int] -> Int
luhn n = (10 - (sum (map normalize (doubleEveryOther (reverse n)))) `mod` 10) `mod` 10
...
toDigits n
    | n < 0 = []
    | n < 10 = [(fromIntegral n)]
    | otherwise = (toDigits ((fromIntegral n) `div` 10)) ++ [((fromIntegral n) `mod` 10)] -- :(
...
map :: (a -> b) -> [a] -> [b]
map _ [] = []
map f (x:xs) = f x : map f xs -- Somehow fromIntegral has to be used here
